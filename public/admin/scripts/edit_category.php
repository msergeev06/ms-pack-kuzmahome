<? include_once(__DIR__."/../../include/header.php"); MSergeev\Core\Lib\Buffer::setTitle('Скрипты - Редактирование категории');
...
{
	if (isset($_POST['title']) && strlen($_POST['title'])>0 && isset($_POST['id']) && intval($_POST['id'])>0)
	{
		Tables\ScriptsCategoriesTable::update(intval($_REQUEST['id']),array("VALUES"=>array('TITLE'=>$_POST['title'])));
...
{
	$arCategory = Tables\ScriptsCategoriesTable::getOne(
		array(
...
<form action="" method="post" enctype="multipart/form-data" name="frmEdit" id="frmEdit" class="form-horizontal">
	<div class="form-group ">
		...
		<div class="col-lg-5"><input type="text" class="form-control " name="title" value="<?=$arCategory['TITLE']?>" required="true"></div>
	</div>
