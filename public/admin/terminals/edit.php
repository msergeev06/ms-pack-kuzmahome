...
{
	$arTerminal = Tables\TerminalsTable::getById(intval($_REQUEST['id']));
}
if (isset($_POST['action']))
{
	...
	if (!$bError)
	{
		$updateID = $arTerminal['ID'];
		unset($arTerminal['ID']);
		Tables\TerminalsTable::update($updateID,array("VALUES"=>$arTerminal));
		$arTerminal['ID'] = $updateID;
	}
}
?>
<br>
...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal" name="frmEdit">
	...
	<input type="hidden" name="id" value="<?=$arTerminal['ID']?>">
	<div class="form-group">
