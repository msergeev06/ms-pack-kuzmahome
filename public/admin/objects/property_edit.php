...
{
	$propertyID = intval($_REQUEST['id']);
}
if ($propertyID==0)
{
	?><span class="text-danger">ID свойства не может равняться 0 (нулю)</span><?
	die();
}
else
{
	$arProperty = Tables\PropertiesTable::getList(
		array(
			'select' => array('ID','TITLE','DESCRIPTION','KEEP_HISTORY','ONCHANGE'),
			'filter' => array('ID' => $propertyID),
			'limit' => 1
		)
	);
	if ($arProperty && isset($arProperty[0]))
	{
		$arProperty = $arProperty[0];
	}
	if (!$arProperty)
	{
		?><span class="text-danger">Ошибка загрузки данных свойства</span><?
		die();
	}
}
if (isset($_POST['action']))
{
	if ($arProperty['TITLE']==$_POST['title'])
	{
		//Сохраняем изменения
		...
		{
			$bShowError = true;
			$textError .= 'Невозможно переименовать свойство, так как свойство с данным именем уже существует<br>';
		}
		else
		{
			//Сохраняем изменения
...
$keep_history = intval($arProperty['KEEP_HISTORY']);
if (isset($_POST['keep_history']))
{
	$keep_history = intval($_POST['keep_history']);
}
$onchange = $arProperty['ONCHANGE'];
			...
			<select name="onchange" class="form-control">
				...
				<?if($arMethods):?>
					<?foreach($arMethods as $arMethod):?>
						<option value="<?=$arMethod['TITLE']?>"<?if($onchange==$arMethod['TITLE']):?> selected<?endif;?>><?=$arMethod['TITLE']?></option>
					<?endforeach;?>
				<?endif;?>
			</select>
