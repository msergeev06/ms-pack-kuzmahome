...
<ul class="nav nav-tabs">
	...
	<li><a href="<?=$curDir?>object_properties_list.php?id=<?=$arObject['ID']?>">Свойства</a></li>
	<li><a href="<?=$curDir?>object_methods_list.php?id=<?=$arObject['ID']?>">Методы</a></li>
</ul><br>
<?if($bShowError):?>
			...
			<select name="class_id" class="form-control " required="true">
				...
				<?if($arClasses):?>
					<?foreach($arClasses as $ar_class):?>
						<option value="<?=$ar_class['TITLE']?>"<?if($classID==$ar_class['ID']):?> selected<?endif;?>><?=$ar_class['TITLE']?></option>
					<?endforeach;?>
				<?endif;?>
			</select>
			</select>
